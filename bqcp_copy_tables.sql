--File Name : bqcp_copy_tables.sql
with table_list_csv as
(
  -- Breakdown a CSV list of tables into an array
  select split('users.user_profiles,products.account_balances,transactions.user_transactions',',') table_list_array
),
table_list as
(
...
(
  ...
  SELECT 'fictional-bank-dev' target_prj_id
  , 'fictional-bank-prd' source_prj_id
),
 copy_commands as
 ( 
